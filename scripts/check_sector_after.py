...
def map_sector(raw: str, name: str = '') -> str:
    r = raw if raw else ''
    n = name if name else ''
    if ('반도체' in r or '전자부품' in r or '집적회로' in r or '디스플레이' in r or '반도체' in n):
        return '반도체'
    ...
    if '자동차' in r or '운송장비' in r or '모터' in r:
        return '자동차'
    if ('은행' in r or '보험' in r or '증권' in r
    ...
    if ('음식' in r or '식품' in r or '음료' in r or '유통' in r
            ...
            or '가정용 기기' in r or '가전' in r or '가구' in r or '출판' in r):
        return '소비재'
...
krx['mapped'] = krx.apply(lambda row: map_sector(row['raw'], row.get('Name', '')), axis=1)
...
print("=" * 60)
checks = [
    ('대덕', None), ('미래반도체', None), ('삼성전자', None),
    ('SK하이닉스', None), ('LG에너지솔루션', None), ('포스코퓨처엠', None),
    ('한화오션', None), ('SK바이오팜', None), ('리가켐바이오', None),
    ('한미반도체', None), ('한울반도체', None), ('카카오게임즈', None),
]
for keyword, _ in checks:
    rows = krx[krx['Name'].str.contains(keyword, na=False)]
    for _, r in rows.head(3).iterrows():
        print(f"  [{r['Name']}] → {r['mapped']}  ({r.get('Industry','')})")

print("\n" + "=" * 60)
print("【'기본' 잔여 Industry 분포 Top 20】")
print("=" * 60)
basic = krx[krx['mapped'] == '기본']
print(f"기본 잔여: {len(basic)}개 (수정 전 899개)")
print(basic['Industry'].value_counts().head(20).to_string())
