...
krx['mapped'] = krx.apply(
    lambda row: map_sector(row['raw'], str(row.get('Name', '') or '')), axis=1
)
...
print("\n=== 주요 종목 확인 ===")
checks = [
    '삼성전자', '대덕', '미래반도체', 'KB금융', '신한지주',
    'LG에너지솔루션', 'SK바이오팜', '한화오션', '한미반도체',
]
for kw in checks:
    rows = krx[krx['Name'].str.contains(kw, na=False)].head(2)
    for _, r in rows.iterrows():
        print(f"  [{r['Name']}] → {r['mapped']}  ({r.get('Industry', '')})")

print("\n=== 기본 잔여 Industry Top 15 ===")
print(basic['Industry'].value_counts().head(15).to_string())
